...
Tool Bridge는 일반 CLI provider와 분리된 opt-in provider입니다. OpenAI Chat Completions 요청의 `tools`와 `tool_choice`를 Claude/Codex CLI의 구조화 출력에 연결하고, 결과를 OpenAI `message.tool_calls`로 반환합니다.

기존 `claude`, `codex` provider의 동작이나 설정은 바뀌지 않습니다. Tool Bridge는 요청마다 일반 CLI 프로세스를 실행하므로 Codex app-server, Claude Agent SDK, 별도 상주 bridge 서버가 필요하지 않습니다.
...
| `driver` | `claude-cli` 또는 `codex-cli` 실행 방식 선택 | 읽기 전용; `config.yaml`에서 변경 |
...
tool-calling 클라이언트에는 `claude-tools` 또는 `codex-tools` 요청 모델명을 지정하고, 기존 서비스는 계속 일반 provider에 연결된 요청 모델명을 사용하면 됩니다. base provider가 `enabled: false`여도 Tool Bridge 항목에 `enabled: true`를 지정해 bridge만 독립적으로 활성화할 수 있습니다.

## 기본 provider와 Tool Bridge를 왜 분리하는가

같은 Claude/Codex CLI를 실행하더라도 맡기는 역할이 다릅니다.

| 구분 | 기본 `claude` / `codex` | `claude-tools` / `codex-tools` bridge |
|---|---|---|
| 주 역할 | 질문에 일반 답변 생성 | 클라이언트가 제공한 함수 중 호출할 함수 선택 |
| API 반환 | `message.content` 텍스트 | OpenAI 형식의 `message.tool_calls` |
| 도구 실행 주체 | CLI 자체 기능일 수 있음 | GenOffice 등 요청을 보낸 클라이언트 |
| 대표 사례 | 요약, 번역, 코드 설명 | 셀 읽기, 문서 수정, 슬라이드 생성 |

예를 들어 GenOffice가 `read_cells` 함수를 요청에 넣고 "A1:B10을 분석해줘"라고 질문했다고 가정합니다. 기본 Codex provider는 "먼저 셀을 읽겠습니다"라는 텍스트를 반환하거나 Codex 자체 도구를 사용하려 할 수 있습니다. GenOffice가 필요한 응답은 다음과 같은 구조입니다.
...
Tool Bridge는 `read_cells`를 직접 실행하지 않습니다. Codex가 함수를 선택하도록 한 뒤 위 호출 정보를 GenOffice에 반환하고, 실제 Excel 접근은 GenOffice가 수행합니다. 이처럼 CLI 자체 도구 권한과 클라이언트 도구 권한의 경계를 분명히 하기 위해 기본 provider와 별도 bridge provider로 둡니다.
...
- 기존 서비스는 일반 Codex를 사용하고 GenOffice만 Tool Bridge를 사용해야 한다면 StarProxy가 두 실행 경로를 구분할 요청 모델명이 필요합니다.
...
- 요청의 도구는 CLI가 직접 실행하지 않습니다. Tool Bridge는 도구 이름과 JSON 인자만 반환하며 실제 실행은 GenOffice 같은 호출자가 담당합니다.
...
사용할 CLI 로그인을 마친 뒤 서버를 시작하고 아래 요청을 보냅니다. Codex를 확인할 때는 모델만 `codex-tools`로 바꿉니다.
